...
- [x] **Day 5** — `search_documents` RAG tool + wired into agent. Embeds the question → `store.query({ topK:4 })` → returns `{ found, results:[{text, source}] }` (loose 0.3 score floor). Agent prompt updated to route (numbers→run_sql, docs→search_documents), ground answers in retrieved text, cite the source, and decline when no doc matches. `npm run eval:day5` = 5/5 live (routing + citation + grounding + SQL). Model kept on `deepseek-v4-pro` (flash also passed 5/5 but pro chosen for harder days).
- [x] **Day 6** — memory. `@mastra/memory@1.26.0` + `LibSQLStore` on the Mastra instance → `data/memory.db`. Two kinds: **message history** (thread-scoped, `lastMessages: 20`) and **working memory** (resource-scoped Markdown template on the agent) holding durable facts (user, currency, fiscal year, anchor date, spend basis). Call signature is `agent.generate(p, { memory: { resource, thread } })` — NOT `{resourceId, threadId}`. `npm run eval:memory` = 5/5 (follow-ups resolve in-thread; working memory crosses threads; history does not). Evals renamed: `eval:sql` / `eval:rag` / `eval:memory`.
- [ ] **Day 7** — guardrails/validation. **Day 8** — evals.
